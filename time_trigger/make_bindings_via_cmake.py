...
binder_executable = f'/usr/local/bin/binder'
...
def make_all_includes():
    ...
    for filename in (glob.glob(f'{this_project_include}/**/*.hpp', recursive=True) +
                     glob.glob(f'{this_project_include}/**/*.cpp', recursive=True) +
                     glob.glob(f'{this_project_include}/**/*.h', recursive=True) +
                     glob.glob(f'{this_project_include}/**/*.cc', recursive=True) +
                     glob.glob(f'{this_project_include}/**/*.c', recursive=True)):
...
def make_bindings_code(all_includes_fn):
    ...
    command = (f'{binder_executable} --root-module {python_module_name} '
               ...
               f'-I{this_project_include} -I{eigen_include} -I{pcl_build_include} -I{pcl_include} -DNDEBUG -v').split()
...
def compile_sources(sources_to_compile):
    ...
    for include_dir in [binder_source, this_project_include, pybind_source, pybind_include, pcl_build_include, pcl_include, eigen_include, get_python_inc()]:
    ...
    lines_to_write.append('target_link_libraries(time_trigger ${PCL_LIBRARIES})')
